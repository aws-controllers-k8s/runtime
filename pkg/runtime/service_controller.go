...
// cluster.
func (c *serviceController) BindControllerManager(ctx context.Context, mgr ctrlrt.Manager, cfg ackcfg.Config) error {
	c.metaLock.Lock()
	defer c.metaLock.Unlock()
	c.cfg = cfg
	...
	for _, rmf := range filteredRMFs {
		...
		go c.lazyBindReconciler(ctx, mgr, rmf, cfg, carmCache, irsCache, exporterInstalled, exporterLogger)
	}
...
// binds the reconciler. Errors are logged rather than returned since this runs
...
func (c *serviceController) lazyBindReconciler(
	...
	for {
		...
		if err != nil {
			log.Error(err, "failed to bind reconciler after CRD became available")
			return
		}
		return
	}
